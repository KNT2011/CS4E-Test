...
public:
    // Hàm t?o
    HanhKhach() : Nguoi("", "", 0), ve(), soluong(0) {}

    HanhKhach(std::string hoten, std::string gioitinh, int tuoi)
        : Nguoi(hoten, gioitinh, tuoi), ve(), soluong(0) {}
    ...
    void Nhap() {
        ...
        ve.resize(soluong);  // S? d?ng resize ?? c?p phát b? nh? cho vector
...
int main() {
    ...
    std::vector<HanhKhach> danhSachHanhKhach(n, HanhKhach());
    ...
    std::sort(danhSachHanhKhach.begin(), danhSachHanhKhach.end(), [](const HanhKhach& a, const HanhKhach& b) {
